...
const Dashboard = () => {
  ...
  const getSessions = async () => {
    ...
    try {
      ...
      const { data: dataGet } = await axios.post(
        "https://neuronex-server.vercel.app/session/all",
        {
          page: 1,
          limit: 50,
          uid: user?.uid,
  ...
  const getFavSessions = async () => {
    ...
    try {
      ...
      const { data: dataGet } = await axios.post(
        "https://neuronex-server.vercel.app/session/favorite",
        {
          page: 1,
          limit: 50,
          uid: user?.uid,
  ...
  return (
    <div className="drawer lg:drawer-open" ref={drawerRef}>
      ...
      <div className="drawer-side  z-30">
        ...
        <ul className=" flex flex-col pt-6 w-[290px] h-full text-lg text-base-content">
          {/* Sidebar content here */}
          <div className="flex lg:hidden flex-row items-center justify-evenly">
            <DrawerToggle />
            <ul className="menu lg:menu-horizontal  text-lg   rounded-box lg:mb-64">
              <li>
                <details>
